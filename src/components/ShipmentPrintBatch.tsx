// components/ShipmentPrintBatch.tsx
...
// مكون لعرض إرسالية واحدة في وضع الطباعة الجماعية
function ShipmentPrintItem({ shipment, index }: { shipment: any; index: number }) {
    ...
    return (
        <div className="print-batch-item">
            <div className="print-page" dir="ltr">
                <div className="batch-header">
                    <span className="batch-number">إرسالية رقم: {shipment?.consignmentNumber ?? "---"}</span>
                    <span className="batch-counter">({index + 1})</span>
                </div>
...
function ShipmentPrintBatch({ shipments, onClear }: Props) {
        ...
        <div className="print-batch-container">
            <div className="batch-controls">
                <span className="batch-count">
                    عدد الإرساليات: {shipments.length}
                </span>
                <button className="print-button" onClick={handlePrint}>
                    🖨 طباعة المجموعة ({shipments.length})
                </button>
                {onClear && (
                    <button className="close-button" onClick={onClear}>
                        ✕ إغلاق
                    </button>
                )}
            </div>

            <div ref={printRef} className="batch-content">
...
export default ShipmentPrintBatch;